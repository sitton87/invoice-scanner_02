"""
ui.py - Graphical User Interface with OCR Support + Hybrid Processor
"""
...
from full_processor_01 import process_full_invoice
...
class InvoiceProcessorGUI:
    ...
    def __init__(self):
        ...
        self.use_hybrid = tk.BooleanVar(value=False)  # New: Hybrid processor option
    ...
    def setup_window(self):
        ...
        self.root.geometry("700x850")  # Increased height for new option
    ...
    def create_widgets(self):
        ...
        main_frame.rowconfigure(9, weight=1)  # Results row will expand (changed from 8 to 9)
...
🚀 Hybrid mode automatically detects PDF content type!"""
        ...
        ocr_radio = ttk.Radiobutton(
            ...
            value=True,
            command=self._update_hybrid_state
        )
        ...
        # NEW: Hybrid processor checkbox
        self.hybrid_check = ttk.Checkbutton(
            mode_column,
            text="🚀 Use Hybrid Processor (Auto-detect PDF type)",
            variable=self.use_hybrid,
            command=self._update_process_info
        )
        self.hybrid_check.pack(anchor=tk.W, pady=(8, 2))
        ...
        # NEW: Processing info label
        self.processing_info_label = ttk.Label(
            main_frame,
            text="",
            font=('Arial', 9),
            foreground='blue',
        ...
        self.processing_info_label.grid(row=3, column=0, columnspan=3, pady=(0, 10))
        ...
        self.select_button.grid(row=4, column=0, columnspan=3, pady=(0, 20))
        ...
        self.file_label.grid(row=5, column=0, columnspan=3, pady=(0, 20))
        ...
        results_frame.grid(row=9, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 15))
        ...
        buttons_frame.grid(row=10, column=0, pady=(10, 0))
    ...
    def _update_hybrid_state(self):
        """Update hybrid checkbox state when OCR/Image mode changes"""
        # When user changes OCR/Image mode, update the info
        self._update_process_info()
    
    def _update_process_info(self):
        """Update processing information label"""
        if self.use_hybrid.get():
            info_text = "🚀 Hybrid Mode: Auto-detects if PDF has selectable text (pdfplumber) or needs OCR"
        else:
            mode_text = "OCR" if self.use_ocr.get() else "Image"
            info_text = f"📍 Standard Mode: {mode_text} processing will be used"
        
        if hasattr(self, 'processing_info_label'):
            self.processing_info_label.config(text=info_text)
    ...
    def _update_process_button_text(self):
        ...
        if hasattr(self, 'process_button'):
            ...
            if self.use_hybrid.get():
                button_text = f"🚀 Process {' + '.join(sections)} (Hybrid)"
            else:
...
"""
    ...
    def process_in_background(self):
        ...
        try:
            ...
            # Select processing method based on user choice
            if self.use_hybrid.get():
                # Use hybrid processor
                try:
                    from hybrid_processor import process_invoice_hybrid
                    result = process_invoice_hybrid(
                        file_path=self.selected_file,
                        progress_callback=update_progress
                    )
                except ImportError as e:
                    # Fallback if hybrid processor not available
                    result = {
                        "success": False,
                        "error": str(e),
                        "message": "Hybrid processor not available. Please ensure hybrid_processor.py exists and all dependencies are installed."
                    }
            else:
                # Use existing full processor
    ...
    def show_results(self, result):
        ...
        # Add processing method used (if available in result)
        ...
        if result["success"]:
            ...
            json_formatted = json.dumps(result['json_data'] if 'json_data' in result else {
                ...
                if k not in ['success', 'message', 'output_file', 'extracted_text', 'processing_info', 'method_used']
            ...
            if self.use_hybrid.get():
                mode_text = f"Hybrid ({result.get('method_used', 'Auto-detect')})"
            else:
                mode_text = "OCR" if self.use_ocr.get() else "Image"
            self.add_to_log(f"🔍 Processing mode: {mode_text}")
            ...
            self.add_to_log("• Try Hybrid mode for better PDF handling")
